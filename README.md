...
* * *
## 历史版本：
1. 1.2.4 版本：增加了为Dialog统一配置主题的功能。
2. 1.2.3 版本：修复了5.0以下不能正常显示进度条的bug。
3. 1.2.2 版本：进行了部分优化。
4. 1.2.1 版本：修复了部分机型会出现软件解析包错误的bug，并增加了网络状态的监听，根据当前的网络状态给出友好的提示，也可手动关闭网络监测。
5. 1.0.1 版本：支持指定版本强制更新。
6. 1.0.0 版本：支持强制更新配置，断点下载，自动恢复。
...
```
dependencies {
    compile 'com.github.kelinZhou:ApkUpdater:1.2.4'
}
```
